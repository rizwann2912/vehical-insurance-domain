    ...
    def __init__(self,region_name=REGION_NAME):
        ...
        if S3Client.s3_client == None or S3Client.s3_resource==None:
            ...
            if __access_key_id is None:
                raise Exception(f'Environmet Variable {AWS_ACCESS_KEY_ID_ENV_KEY} is not set')
            if __secret_access_key is None:
                raise Exception(f'Environment Varibale {AWS_SECRET_ACCESS_KEY_ENV_KEY} is not set')
